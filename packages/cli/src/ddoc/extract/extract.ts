...
const DesignDocumentSchema = Type.Object({
  ...
  options: Type.Optional(
    Type.Object({
      local_seq: Type.Optional(Type.Boolean()),
      include_design: Type.Optional(Type.Boolean()),
    }),
  ),
//  views: Type.Optional(Type.Map(ViewsSchema)),

  views: Type.Optional((ViewsSchema)),
  updates: Type.Optional(MapSchema),
  filters: Type.Optional(MapSchema),
  validate_doc_update: Type.Optional(Type.String()),
  ...
  try {
    ...
    if (opts.format === 'json') {
      ...
      for (const doc of designDocuments) {
        ...
        if (doc.updates) {
          stats.updates:any = stats.updates + 1
      ...
      for (const doc of designDocuments) {
        const filename = `${doc._id?.split('_design/')[1]}-${doc._rev}.${opts.format === 'js' ? 'js' : 'ts'
          }`
    ...
    console.error(chalk.bgRed(chalk.white(` ${(err as Error).message} `)))
    process.exit(1)
  }
}
